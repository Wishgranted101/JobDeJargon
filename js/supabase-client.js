...
async function checkUserSession() {
    ...
    if (session) {
        ...
        // ✅ CRITICAL FIX: Dispatch event after profile loads
        window.dispatchEvent(new Event('userSessionChanged'));
        console.log('✅ userSessionChanged event dispatched');
    } else {
...
function updateUserUI() {
    ...
    if (creditDisplay && window.currentUser) {
        creditDisplay.textContent = `💳 ${window.currentUser.credits} credits`;
    }
